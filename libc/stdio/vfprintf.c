...
extern int _ppfs_parsespec(ppfs_t *ppfs)
{
  ...
  __attribute__ ((unused)) int argnumber[3];			/* width, precision, 1st data arg */
...
#endif
#endif /* __UCLIBC_MJN3_ONLY__ */
			s = _uintmaxtostr(buf + sizeof(buf) - 1, (uintmax_t) _load_inttype(*argtype & __PA_INTMASK, *argptr, base), base, (__UIM_CASE)alphacase);
...
int VFPRINTF (FILE * __restrict stream, register const FMT_TYPE * __restrict format, va_list arg)
{
	ppfs_t ppfs;
	int count, r;
