...
_stdlib_strto_ll(register const char * __restrict str, char ** __restrict endptr, int base, int sflag);
...
_stdlib_strto_l(register const char * __restrict str, char ** __restrict endptr, int base, int sflag);
...
struct scan_cookie {
	...
	//MOSS
	char input_buffer[MAX_READ_LEN] ;
...
int getc_scanf(register struct scan_cookie* sc)
{
    ...
    if(sc->input_buffer[i] == '\0')
    {
        sc->input_buffer_index = 0 ;
        return EOF ;
    }

    sc->input_buffer_index++ ;
...
extern void __init_scan_cookie(register struct scan_cookie *sc, register FILE *fp);
...
void __init_scan_cookie(register struct scan_cookie *sc, register FILE *fp)
{
	sc->fp = fp;
	sc->nread = 0;
	sc->ungot_flag = 0;
	...
	sc->fake_decpt = sc->decpt = (const unsigned char*)decpt_str;
	sc->decpt_len = 1;
...
#define valid_digit(cc,base) (isxdigit(cc) && ((base == 16) || (cc - '0' < base)))
	...
	if (psfs->conv_num <= CONV_i) { /* integer conversion */
		...
		*b = 0;						/* null-terminate */
		...
		if (psfs->store) {
			...
			++psfs->cnt;
			_store_inttype(psfs->cur_ptr, psfs->dataargtype, (uintmax_t)STRTOUIM((const char*)buf, NULL, base, 1-usflag));
		}
		return 0;
	}
 ...
 NO_STARTING_DIGIT:
	...
	do {
		if (!*p) {
			strcpy((char*)b, (const char*)sc->decpt);
			b += sc->decpt_len;
			break;
		}
		if (*p != sc->cc) {
